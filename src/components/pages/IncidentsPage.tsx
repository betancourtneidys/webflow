...
export function IncidentsPage({ lang }: { lang: Lang }) {
    ...
    <main className="flex min-h-screen flex-col">
      ...
      <div className="mx-auto w-full max-w-6xl flex-1 px-4 pb-20 pt-10 sm:px-6">
        <div className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.16em] text-critical">
          ...
          {t.sim.listHeader(incidents.length)}
        </div>
        <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">{t.list.title}</h1>
        <p className="mt-2 max-w-xl text-muted">{t.list.subtitle}</p>

        <ul className="mt-10 grid gap-4 md:grid-cols-3">
          {incidents.map((incident, i) => (
            <li key={incident.id}>
              <Link
                href={localePath(lang, `/incident/${incident.id}`)}
              ...
              >
                ...
                <p className="mt-1 text-[13px] text-muted">
                  <span className="capitalize">{t.severity[incident.severity]}</span> · {t.list.started(incident.startedAgo)}
                </p>
                <div className="mt-3">
                ...
                <div className="mt-auto flex items-center justify-between pt-6 text-sm">
                  <span className="text-faint">{t.list.resources(incident.resources.length)}</span>
